...
#define lor_ips "./inputs/loremipsum.txt"
#define out_pts "./outputs/pts.txt"
...
string to_lower(string s){																	//Funkcja do konwersji stringa na taki, ktory sklada sie wylacznie z malych liter
    transform(s.begin(), s.end(), s.begin(), [](unsigned char c){ return to_lower(c); });
    return s;
}
...
char losowyZamiennik(const set<char>& zamienniki){
...
string zamienLitery(const string& tekst, const unordered_map<char, set<char>>& mapa, double procent){
    string wynik = tekst;
    ...
    random_shuffle(indeksy.begin(), indeksy.end());								// Losowe tasowanie indeksów
...
int main(){
    ...
    if(!points_file.is_open()){
        cerr << "Nie można otworzyć pliku do zapisu pointów." << '\n';
        return 1;
    }
    ...
    // Algorytm Kruskala
    UnionFind uf(number_of_points);
    vector<tuple<int, int, int, int>> mst; // Minimalne drzewo rozpinające (x1, y1, x2, y2)
    ...
    ofstream roads_file("drogi.txt");
    if(!roads_file.is_open()){
        cerr << "Nie można otworzyć pliku do zapisu dróg." << '\n';
        return 1;
    }
    ...
    roads_file.close();
    cout << "Wygenerowano " << number_of_points << " pointow oraz " << mst.size() << " drog.\n";

    // Generowanie tragarzy
    ofstream tragarze_file("tragarze.txt");
    if(!tragarze_file.is_open()){
        cerr << "Nie można otworzyć pliku do zapisu tragarzy.\n";
        return 1;
    }

    int minTragarze = config["MIN_ILOSC_TRAGARZY"];
    int maxTragarze = config["MAX_ILOSC_TRAGARZY"];
    int liczbaTragarzy = rand() % (maxTragarze - minTragarze + 1) + minTragarze;
    tragarze_file << liczbaTragarzy << '\n';

    // We'll store the preferences of each tragarz in a vector of sets to ensure mutual preferences
    vector<set<int>> preferences(liczbaTragarzy + 1);
    ...
    for (int i = 1; i <= liczbaTragarzy; i++){
        ...
        for (int j = i + 1; j <= liczbaTragarzy; j++){
            if(rand() % 2 == 0){ // Randomly decide if they like each other
    ...
    tragarze_file.close();
    cout << "Wygenerowano " << liczbaTragarzy << " tragarzy." << '\n';
    ...
    file.close();
    cout << "Wygenerowano opowiesc-melodie 'text.txt'." << '\n';

    string text_without_interpunction = rmv_interpunction(return_lorem_ipsum()); 		//Usuwanie znaków interpunkcyjnych

    // Zbiór do przechowywania unikalnych słów
    set<string> unique_words;

    // String stream do podziału tekstu na słowa
    stringstream ss(text_without_interpunction);
    string slowo;

    // Podział tekstu na słowa i dodanie ich do zbioru
    while(ss >> slowo){
        unique_words.insert(slowo);
    }

    // Otwarcie pliku do zapisu
    ofstream plik("slownik.txt");
    ...
    if(plik.is_open()){
        ...
        plik.close();
        cout << "Slowa zostaly zapisane do pliku slownik.txt." << '\n';
    } else{
        cerr << "Nie można otworzyć pliku do zapisu." << '\n';
    }
    
    ifstream plikTekst("text.txt");
    ofstream plikWynik("tekstzpoli.txt");

    if(!plikTekst.is_open()){
        cerr << "Nie można otworzyć pliku text.txt." << '\n';
        return 1;
    }

    if(!plikWynik.is_open()){
        cerr << "Nie mozna otworzyc pliku tekstzpoli.txt." << '\n';
        return 1;
    }
    ...
    auto mapaOdwracen = mirrored_letters();

    // Przetwarzanie każdej linii z tekstu
    while(getline(plikTekst, linia)){
        string liniaZamieniona = zamienLitery(linia, mapaOdwracen, procentZamiany);
    ...
    cout << "Przetwarzanie zakonczone. Wynik zapisany w pliku tekstzpoli.txt." << '\n';
