...
struct Point {
  double x, y;
  int label; // Dodajemy etykietę dla punktu
};
...
bool compareByAngle(const Point & p1,
  const Point & p2) {
  // Obliczamy kąt każdego punktu względem osi X
  double angle1 = atan2(p1.y - minY, p1.x - minX);
  double angle2 = atan2(p2.y - minY, p2.x - minX);

  // Upewniamy się, że kąty mieszczą się w przedziale od 0 do 2*pi
  if (angle1 < 0) angle1 += 2 * M_PI;
  if (angle2 < 0) angle2 += 2 * M_PI;

  // Sortujemy zgodnie z ruchem wskazówek zegara
...
int findIndexInPoints(const Point & point,
  ...
  return -1; // Zwróć -1 jeśli punkt nie jest znaleziony
}
...
int main(int argc, char * argv[]) {
  ...
  }; // Kolor dla normalnych odcinków otoczki
  SDL_Color roadColor = {
  ...
  // Znajdź pary tragarzy o różnych typach i obustronnej sympatii
  ...
  for (size_t i = 0; i < porters.size(); ++i) {
    if (!tragarzDostepny[i]) {
      continue; // Jeśli tragarz nie jest dostępny, przejdź do następnego
    }
    ...
    for (size_t j = i + 1; j < porters.size(); ++j) {
      ...
      if (porters[i].typ != porters[j].typ) {
        ...
        if (sympatiaAtoB && sympatiaBtoA) {
          ...
          break; // Znaleziono parę, przejdź do następnego tragarza
        }
      }
    }
  }
  ...
  outFile << paryTragarzy.size() << std::endl; // Zapisz liczbę par tragarzy w pierwszej linii
  ...
  if (kosztFile.is_open()) {
    ...
    // Nadajemy etykiety punktom należącym do otoczki wypukłej
    //labelConvexHullPoints(hull, renderer, {255, 255, 255, 255}, font);

    do {
      closestIndex = findIndexInPoints(closestPoint, points);
      graph = createGraph(points, roads); // Utwórz graf z aktualnymi punktami i drogami
      path = dijkstra(graph, closestIndex, selectedIndex); // Szukaj ścieżki
      ...
      if (distanceToNextPoint <= 1) {
        ...
        if (!intersections.empty()) {
          ...
          if (!(validNextPoint.x == currentPoint.x && validNextPoint.y == currentPoint.y)) {
            points.push_back(validNextPoint); // Dodaj nowy punkt do zbioru punktów
            roads.push_back({
              currentPoint,
              validNextPoint
            }); // Utwórz nową drogę
  ...
  if (!inFile.is_open()) {
    std::cerr << "Nie można otworzyć pliku koszt.txt do odczytu." << std::endl;
    return 654;
  }
  ...
  // Odczytaj wszystkie wartości z pliku i oblicz ich sumę
  while (inFile >> value) {
    totalSum += (value * 2); // tragarz musi też wrócić, więc odległość razy 2
  ...
  // Teraz otwórz ten sam plik do dopisywania (append mode)
  std::ofstream outResultFile("result.txt", std::ios::out);
  if (!outResultFile.is_open()) {
    std::cerr << "Nie można otworzyć pliku koszt.txt do zapisu." << std::endl;
    return 561;
  }

  // Dopisz sumę na końcu pliku
  ...
  // Wektor, który przechowa sumaryczną odległość pokonaną przez każdą parę tragarzy
  std::vector < double > porterSums(numberOfPorters, 0);
  // Wektor przechowujący listy zadań dla każdej pary tragarzy
  std::vector < std::vector < double >> porterTasks(numberOfPorters);

  // Przypisywanie zadań od największego do najmniejszego
  bool assignFromStart = true; // Flag, aby przełączać przypisywanie z początku i końca listy distances
  for (int i = distances.size() - 1; i >= 0; --i) {
    // Znajdź parę tragarzy z najmniejszą sumą odległości
    auto minElementIter = std::min_element(porterSums.begin(), porterSums.end());
    int minIndex = std::distance(porterSums.begin(), minElementIter);

    // Przypisz zadanie do tej pary tragarzy
    porterTasks[minIndex].push_back(distances[i]);
    porterSums[minIndex] += distances[i];
  }

  // Wypisz rezultaty
  for (int i = 0; i < numberOfPorters; ++i) {
    if (porterSums[i] > maks) {
      maks = porterSums[i];
    }
  }
