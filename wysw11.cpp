...
bool areSegmentsEqual(const Point& a1, const Point& b1, const Point& a2, const Point& b2) {
    return (a1.x == a2.x && a1.y == a2.y && b1.x == b2.x && b1.y == b2.y) ||
           (a1.x == b2.x && a1.y == b2.y && b1.x == a2.x && b1.y == a2.y);
}

// Function to draw a dashed line alternating between two colors
void drawDashedLine(SDL_Renderer* renderer, int x1, int y1, int x2, int y2,
                    const SDL_Color& color1, const SDL_Color& color2, double dashLength) {
    // Calculate the distance between points
    double distance = sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
    double gapLength = dashLength;  // Length of gaps between dashes, same as dash length

    // Calculate the unit vector components along the line
    double dx = (x2 - x1) / distance;
    double dy = (y2 - y1) / distance;

    double currentLength = 0.0;  // Tracks the length drawn so far
    bool useColor1 = true;       // Toggle between color1 and color2

    while (currentLength < distance) {
        // Calculate the end point of the next segment
        double segmentLength = std::min(dashLength, distance - currentLength);
        double endX = x1 + (currentLength + segmentLength) * dx;
        double endY = y1 + (currentLength + segmentLength) * dy;

        // Set the color for the current segment
        SDL_SetRenderDrawColor(renderer, (useColor1 ? color1.r : color2.r),
                               (useColor1 ? color1.g : color2.g),
                               (useColor1 ? color1.b : color2.b), 255);

        // Draw the current segment
        SDL_RenderDrawLine(renderer, int(x1 + currentLength * dx), int(y1 + currentLength * dy),
                           int(endX), int(endY));

        // Prepare for the next segment
        currentLength += segmentLength;
        useColor1 = !useColor1;  // Switch colors
    }
}
...
int main(int argc, char* argv[]) {
    ...
    SDL_Color convexColor = {static_cast<Uint8>(config["CONVEX_COLOR_R"]), static_cast<Uint8>(config["CONVEX_COLOR_G"]),
		static_cast<Uint8>(config["CONVEX_COLOR_B"]), static_cast<Uint8>(config["CONVEX_COLOR_A"])};  // Kolor dla normalnych odcinków otoczki
    SDL_Color roadColor = {static_cast<Uint8>(config["ROAD_COLOR_R"]), static_cast<Uint8>(config["ROAD_COLOR_G"]),
		static_cast<Uint8>(config["ROAD_COLOR_B"]), static_cast<Uint8>(config["ROAD_COLOR_A"])};  // Kolor dla normalnych odcinków otoczki
    SDL_Color pointColor = {static_cast<Uint8>(config["POINT_COLOR_R"]), static_cast<Uint8>(config["POINT_COLOR_G"]),
		static_cast<Uint8>(config["POINT_COLOR_B"]), static_cast<Uint8>(config["POINT_COLOR_A"])};  // Kolor dla normalnych odcinków otoczki
...
for (int i = 0; i < hull.size(); i++) {
    ...
    for (const auto& road : roads) {
        if (areSegmentsEqual(hull[i], hull[next], road.start, road.end)) {
            drawDashedLine(renderer, start_x, start_y, end_x, end_y, convexColor, roadColor, config["DASH_LENGTH"]);
            isRoad = true;
            break;
        }
    }
