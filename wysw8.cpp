...
// Funkcja do rysowania przerywanej linii
void drawDashedLine(SDL_Renderer* renderer, int x1, int y1, int x2, int y2, const SDL_Color& color1) {
    SDL_SetRenderDrawColor(renderer, color1.r, color1.g, color1.b, color1.a);
    for (int i = 0; i < 300; i += 20) {
        SDL_RenderDrawLine(renderer, x1 + (x2 - x1) * i / 300, y1 + (y2 - y1) * i / 300,
                           x1 + (x2 - x1) * (i + 10) / 300, y1 + (y2 - y1) * (i + 10) / 300);
        if (i % 40 != 20) {
            SDL_SetRenderDrawColor(renderer, color1.r, color1.g, color1.b, color1.a);
        }
    }
...
for (const auto& road : roads) {
    ...
    start_y = window_height + 2 * margin - start_y;  // Invert y-axis to align with SDL's graphic coordinate system
	...
	// Draw the convex hull (linią przerywaną)
    SDL_SetRenderDrawColor(renderer, 0, 255, 0, 255);  // Green color for the hull
    for (int i = 0; i < hull.size(); i++) {
        int next = (i + 1) % hull.size();
        int start_x = margin + (int)(((hull[i].x - config["MIN_WSPOLRZEDNA"]) / (double)(config["MAX_WSPOLRZEDNA"] - config["MIN_WSPOLRZEDNA"])) * window_width);
