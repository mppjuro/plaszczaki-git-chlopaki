...
std::string zamienLitery(const std::string& tekst, const std::unordered_map<char, std::set<char>>& mapa, double procent) {
    ...
    // Zamiana liter na ich odpowiedniki
    for (size_t i = 0; i < liczbaDoZamiany; ++i) {
...
int generator() {
    ...
    std::ofstream roads_file(ROADS_FILE);
    ...
    std::ofstream tragarze_file(PORTERS_FILE);
    ...
    std::string tekstBezInterpunkcji = usunInterpunkcje(generateLoremIpsum());
    ...
    // Wczytywanie tekstu
    std::ifstream plikTekst(INPUT_TEXT);
    std::ofstream plikWynik(MODIFIED_TEXT);

    if (!plikTekst.is_open()) {
        std::cerr << "Nie można otworzyć pliku text.txt." << std::endl;
        return 1;
    }
    ...
    while (std::getline(plikTekst, linia)) {
        ...
        plikWynik << liniaZamieniona << std::endl;
    }
