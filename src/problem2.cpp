...
int problem2() {
    ...
    std::ifstream plikSlownik(DICT_FILE);
    std::string slowo;
    ...
    std::ifstream plikTekst(MODIFIED_TEXT);
    std::ofstream plikWynik(PROCESSED_TEXT);
    std::string linia;
    ...
    ifstream inFile(PROCESSED_TEXT);
    if (!inFile.is_open()) {
    ...
    saveDictionaryAndTransformedText(OUT_TXT, finalDictionary, transformedText);
    ...
    std::ifstream inputFile(PROCESSED_TEXT, std::ios::in);
    std::ofstream outputFile(KOD_TXT, std::ios::out);
    ...
    std::ifstream inputFileout(OUT_TXT, std::ios::in);
    std::ofstream outputFilekodowanyout(KOD_OUT, std::ios::out);
    ...
    ifstream inputKodowany(KOD_TXT);
    if (!inputKodowany.is_open()) {
        cerr << "Error opening file kodowanykomp.txt" << endl;
        return 1;
    }

    ofstream outputKodowany(OUT_BIN, ios::out | ios::binary);
...
*/
    ifstream inputKodowanyKomp(KOD_OUT);
    if (!inputKodowanyKomp.is_open()) {
        cerr << "Error opening file kodowanyout.txt" << endl;
        return 1;
    }

    ofstream outputKodowanyKomp(OUT_KOMP, ios::out | ios::binary);
    ...
    ifstream inputKodowanyTest(OUT_BIN, ios::binary | ios::ate);
    ifstream inputKodowanyKompTest(OUT_KOMP, ios::binary | ios::ate);
    ...
    ifstream binaryInput(OUT_KOMP, ios::binary);
    ofstream textOutput(KOD_TXT2);
    ...
    ifstream input(KOD_TXT2, ios::binary);
    ofstream output(OUT_TXT2);
