    ...
    for (const auto& road : roads) {
        ...
        if (start_index != -1 && end_index != -1) {
            ...
            graph[end_index].push_back({start_index,
                                        dist});  // Assuming undirected graph
        }
    }
    return graph;
}
...
int main(int argc, char* argv[]) {
    ...
    SDL_Color convexColor = {
        ...
        static_cast<Uint8>(config["CONVEX_COLOR_A"])};  // Kolor dla normalnych odcinkĂłw otoczki
    SDL_Color roadColor = {
        static_cast<Uint8>(config["ROAD_COLOR_R"]),
        static_cast<Uint8>(config["ROAD_COLOR_G"]),
        static_cast<Uint8>(config["ROAD_COLOR_B"]),
        static_cast<Uint8>(config["ROAD_COLOR_A"])};  // Kolor dla normalnych odcinkĂłw otoczki
    SDL_Color pointColor = {
        ...
        static_cast<Uint8>(config["POINT_COLOR_A"])};  // Kolor dla normalnych odcinkĂłw otoczki
    ...
    for (auto& point : points) {
        ...
        SDL_Rect rect = {
            ...
            render_y - 5,
            10,
            10};  // Draw slightly larger points
        ...
        for (size_t j = i + 1; j < porters.size(); ++j) {
            ...
            if (porters[i].typ != porters[j].typ) {
                ...
                if (sympatiaAtoB && sympatiaBtoA) {
                    paryTragarzy.push_back({porters[i].nr,
                                            porters[j].nr});
            ...
            if (distanceToNextPoint <= 1) {
                ...
                if (!intersections.empty()) {
                    ...
                    if (!(validNextPoint.x == currentPoint.x && validNextPoint.y == currentPoint.y)) {
                        ...
                        roads.push_back({currentPoint,
                                         validNextPoint});  // UtwĂłrz nowÄ… drogÄ™
