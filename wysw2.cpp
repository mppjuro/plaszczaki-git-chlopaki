...
int main(int argc, char* argv[]) {
    ...
    while (plik >> x >> y) {
        int render_x = (int)(((x - config["MIN_WSPOLRZEDNA"]) / (double)(config["MAX_WSPOLRZEDNA"] - config["MIN_WSPOLRZEDNA"])) * window_width);
        int render_y = (int)(((y - config["MIN_WSPOLRZEDNA"]) / (double)(config["MAX_WSPOLRZEDNA"] - config["MIN_WSPOLRZEDNA"])) * window_height);
        ...
        SDL_Rect textRect = {render_x + 10, render_y, textSurface->w, textSurface->h};
