...
void drawDashedLine(SDL_Renderer* renderer, int x1, int y1, int x2, int y2, 
    ...
    // Calculate the full dash cycle length (dash + gap)
    double cycleLength = dashLength + gapLength;

    // Calculate the number of dashes to draw
    int numDashes = int(distance / cycleLength);

    // Calculate the unit vector components along the line
    double dx = (x2 - x1) / distance;
    double dy = (y2 - y1) / distance;

    // Draw each dash and gap segment
    for (int i = 0; i < numDashes; ++i) {
        // Starting point of the dash
        double startX = x1 + i * cycleLength * dx;
        double startY = y1 + i * cycleLength * dy;

        // Ending point of the dash
        double endX = startX + dashLength * dx;
        double endY = startY + dashLength * dy;

        // Set color and draw the dash
        ...
        // Starting point of the gap
        startX = endX;
        startY = endY;

        // Ending point of the gap
        endX = startX + dashLength * dx;
        endY = startY + dashLength * dy;

        // Set the second color and draw the gap
        if (i < numDashes - 1) { // Avoid drawing beyond the segment
...
int main(int argc, char* argv[]) {
	SDL_Color convexColor = {0, 255, 0, 255};  // Kolor dla normalnych odcinków otoczki
    SDL_Color roadColor = {0, 255, 255, 255};  // Kolor dla normalnych odcinków otoczki
    SDL_Color pointColor = {255, 0, 0, 255};  // Kolor dla normalnych odcinków otoczki
...
for (const auto& road : roads) {
	...
	/*
	// Draw the dashed line convex hull only on roads on covex hull
    ...
    for (int i = 0; i < hull.size(); i++) {
        ...
        drawDashedLine(renderer, start_x, start_y, end_x, end_y, green);
    }
	*/
	//display
